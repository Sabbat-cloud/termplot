...
impl BrailleCanvas {
    ...
    /// Reemplaza celdas completas cuando `top` tiene contenido. Esto permite
    /// superponer curvas encima de una rejilla sin mezclar ambos glyphs.
    ...
    pub(crate) fn overlay_without_background(&mut self, top: &BrailleCanvas, background_mask: &[u8]) {
        ...
        for idx in 0..self.buffer.len() {
            ...
            let existing_foreground = self.buffer[idx] & !background_mask[idx];

            self.buffer[idx] &= !background_mask[idx];
            self.buffer[idx] |= top.buffer[idx];
