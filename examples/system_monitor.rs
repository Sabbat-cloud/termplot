...
// CORRECCIÓN: Eliminados CpuExt y SystemExt, ya no existen en sysinfo 0.30+
use sysinfo::System;
...
fn main() -> io::Result<()> {
    ...
    while running {
        ...
        // --- C. LÓGICA DE SISTEMA (Cada 500ms) ---
        if last_sys_update.elapsed() >= Duration::from_millis(500) {
            ...
            // Evitar división por cero si total_mem es 0 (raro pero posible en virtualización)
        ...
        // --- D. LÓGICA DE PARTÍCULAS (Simulación de estrés) ---
        let w_px = chart.canvas.pixel_width() as f64;
        let h_px = chart.canvas.pixel_height() as f64;
        // Definimos el área de simulación (Mitad inferior)
        ...
        // Física y Renderizado de partículas
        ...
        let draw_mini_chart = |chart: &mut ChartContext, data: &VecDeque<f64>, x_offset: f64, width: f64, color: Color| {
            ...
            let mut prev_y = height - (data[0] / 100.0 * height); // 100.0 es el max valor (100%)
