...
fn main() -> io::Result<()> {
    ...
    loop {
        ...
        if show_help {
            ...
            let stress = format!(
                "Monitor: {} FPS | Latencia: {}ms | Cuerpos: {}/150",
                current_fps,
                ms,
                bodies.len()
            );
            ...
            let control_txt = format!(
                "Tiempo: {:.2} | LOD: {} | Zoom: {:.1} | Pulsa [H] para Ayuda",
        ...
        print!(
            ...
            chart_ref
                .canvas
                .render_with_options(true, Some("Simulador FÍSICO Real (N-Cuerpos - Leapfrog)"))
                .replace('\n', "\r\n")
        );
        stdout.flush()?;
