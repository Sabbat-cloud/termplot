...
impl ZBuffer {
    fn new(w: usize, h: usize) -> Self { Self { w, h, z: vec![f64::INFINITY; w * h], id: vec![None; w * h] } }
    fn clear(&mut self) { self.z.fill(f64::INFINITY); self.id.fill(None); }
    ...
    fn test_and_set(&mut self, x: usize, y: usize, depth: f64, body_id: Option<usize>) -> bool {
        let i = self.idx(x, y);
        if depth < self.z[i] { self.z[i] = depth; self.id[i] = body_id; true } else { false }
    }
}

// ============================================================================
// GRAVEDAD: FÍSICA NEWTONIANA DE N-CUERPOS
// ============================================================================
const G: f64 = 1.0; 

struct PhysicsBody {
    name: String, color: Color, radius: f64, is_star: bool,
    mass: f64, pos: Vec3, vel: Vec3, force: Vec3,
    rot_angle: f64, rot_rate: f64,
    trail: VecDeque<Vec3>, 
}
...
fn get_circular_orbit(center_mass: f64, center_pos: Vec3, center_vel: Vec3, distance: f64) -> (Vec3, Vec3) {
...
fn create_body(name: &str, c: Color, r: f64, star: bool, m: f64, p: Vec3, v: Vec3, rot: f64) -> PhysicsBody {
    PhysicsBody {
        name: name.to_string(), color: c, radius: r, is_star: star, mass: m,
        pos: p, vel: v, force: Vec3::new(0.0,0.0,0.0), rot_angle: 0.0, rot_rate: rot,
...
fn project_to_screen(v_cam: Vec3, w: f64, h: f64, scale: f64) -> Option<(isize, isize, f64)> {
    if v_cam.z <= 1.5 { return None; } // Culling de cámara
...
fn plot_z(chart: &mut ChartContext, zb: &mut ZBuffer, x: isize, y: isize, z: f64, col: Color, id: Option<usize>) {
    ...
    if ux < zb.w && uy < zb.h && zb.test_and_set(ux, uy, z, id) { chart.canvas.set_pixel_screen(ux, uy, Some(col)); }
}

fn line_z(chart: &mut ChartContext, zb: &mut ZBuffer, p1: (isize, isize, f64), p2: (isize, isize, f64), col: Color) {
    let min_x = p1.0.min(p2.0); let max_x = p1.0.max(p2.0);
    let min_y = p1.1.min(p2.1); let max_y = p1.1.max(p2.1);

    if max_x < 0 || min_x >= zb.w as isize || max_y < 0 || min_y >= zb.h as isize { return; }

    let dx = (p2.0 - p1.0).abs(); let dy = (p2.1 - p1.1).abs(); 
    ...
    for s in 0..=steps {
        ...
        plot_z(chart, zb, xf.round() as isize, yf.round() as isize, zf, col, None);
    }
}
...
fn main() -> io::Result<()> {
    ...
    // --- SETUP DEL SISTEMA GRAVITACIONAL ---
    let mut bodies = Vec::new();
    
    let sun_mass = 10000.0;
    let sun_pos = Vec3::new(0.0, 0.0, 0.0);
    let sun_vel = Vec3::new(0.0, 0.0, 0.0);
    bodies.push(create_body("Sol", Color::BrightYellow, 2.5, true, sun_mass, sun_pos, sun_vel, 0.05));

    let earth_mass = 50.0;
    let (e_pos, e_vel) = get_circular_orbit(sun_mass, sun_pos, sun_vel, 30.0);
    bodies.push(create_body("Tierra", Color::Blue, 0.8, false, earth_mass, e_pos, e_vel, 0.8));

    let (m_pos, m_vel) = get_circular_orbit(earth_mass, e_pos, e_vel, 2.5);
    bodies.push(create_body("Luna", Color::White, 0.3, false, 0.5, m_pos, m_vel, 0.2));
    ...
    bodies.push(create_body("Júpiter", Color::BrightMagenta, 1.8, false, jup_mass, j_pos, j_vel_elliptical, 1.5));

    // Guardamos la cantidad original de cuerpos (para el comando Limpiar Asteroides 'X')
    let base_body_count = bodies.len();

    // Variables de Estado de la Simulación
    let mut cam_pos = Vec3::new(0.0, -30.0, -100.0);
    let mut cam_pitch = 0.3_f64; let mut cam_yaw = 0.0_f64;
    ...
    loop {
        ...
        if (cols, rows) != last_term || chart.is_none() {
            last_term = (cols, rows);
            let new_chart = ChartContext::new((cols as usize).saturating_sub(4), (rows as usize).saturating_sub(6));
            zbuf = Some(ZBuffer::new(new_chart.canvas.pixel_width(), new_chart.canvas.pixel_height()));
            chart = Some(new_chart);
            regen_mesh = true;
        }
        if regen_mesh { sphere_pts = make_sphere_points((10 + detail as usize * 4).min(140), (20 + detail as usize * 8).min(260)); regen_mesh = false; }
            ...
            if let Event::Key(KeyEvent { code, .. }) = event::read()? {
                match code {
                    KeyCode::Char('q') | KeyCode::Esc => {
                        execute!(stdout, cursor::Show, terminal::Clear(ClearType::All), DisableMouseCapture)?;
                        terminal::disable_raw_mode()?; return Ok(());
                    }
                    KeyCode::Char('w') => { cam_pos.z += 2.0 * cam_yaw.cos(); cam_pos.x += 2.0 * cam_yaw.sin(); follow_body = None; },
                    KeyCode::Char('s') => { cam_pos.z -= 2.0 * cam_yaw.cos(); cam_pos.x -= 2.0 * cam_yaw.sin(); follow_body = None; },
                    KeyCode::Char('a') => { cam_pos.x -= 2.0 * cam_yaw.cos(); cam_pos.z += 2.0 * cam_yaw.sin(); follow_body = None; },
                    KeyCode::Char('d') => { cam_pos.x += 2.0 * cam_yaw.cos(); cam_pos.z -= 2.0 * cam_yaw.sin(); follow_body = None; },
                    KeyCode::Char('e') => cam_pos.y += 1.5, KeyCode::Char('c') => cam_pos.y -= 1.5,
                    
                    KeyCode::Left => cam_yaw += 0.1, KeyCode::Right => cam_yaw -= 0.1,
                    KeyCode::Up => cam_pitch += 0.1, KeyCode::Down => cam_pitch -= 0.1,
                    
                    KeyCode::Char('u') => time_scale -= 0.05, KeyCode::Char('i') => time_scale += 0.05,
                    KeyCode::Char('+') | KeyCode::Char('=') => zoom = (zoom * 1.1).min(20.0),
                    KeyCode::Char('-') => zoom = (zoom / 1.1).max(0.1),
                    KeyCode::Char('m') => { detail = (detail + 1).min(10); regen_mesh = true; },
                    KeyCode::Char('n') => { detail = (detail - 1).max(1); regen_mesh = true; },
                    
                    KeyCode::Char('o') => show_orbits = !show_orbits,
                    KeyCode::Char('f') => { follow_body = selected_body; }
                    KeyCode::Char('h') | KeyCode::Char('H') => show_help = !show_help,
                    
                    // BOTÓN DEL PÁNICO: Limpiar Asteroides
                    KeyCode::Char('x') | KeyCode::Char('X') => {
                        bodies.truncate(base_body_count);
                        asteroide_id_counter = 1;
                        if let Some(id) = selected_body { if id >= base_body_count { selected_body = None; } }
                        if let Some(id) = follow_body { if id >= base_body_count { follow_body = None; } }
                    }

                    KeyCode::Char(' ') => {
                        if time_scale == 0.0 { time_scale = saved_time_scale; }
                        else { saved_time_scale = time_scale; time_scale = 0.0; }
                    }
                    _ => {}
                }
            } else if let Event::Mouse(me) = event::read()? {
                let get_clicked_id = |c: u16, r: u16, zb: &ZBuffer| -> Option<usize> {
                    let mx = (c.saturating_sub(2) as isize) * 2; let my = (r.saturating_sub(2) as isize) * 4;
                    let mut cl_id = None; let mut min_dist = 40.0; let mut close_z = f64::INFINITY;
                    for py in (my-40).max(0) as usize..=(my+40).min(zb.h as isize-1) as usize {
                        for px in (mx-40).max(0) as usize..=(mx+40).min(zb.w as isize-1) as usize {
                            if let Some(id) = zb.id[zb.idx(px, py)] {
                                let dist = ((px as f64 - mx as f64).powi(2) + (py as f64 - my as f64).powi(2)).sqrt();
                                if dist < min_dist { min_dist = dist; close_z = zb.z[zb.idx(px, py)]; cl_id = Some(id); }
                                else if (dist - min_dist).abs() < 1.0 && zb.z[zb.idx(px, py)] < close_z {
                                    close_z = zb.z[zb.idx(px, py)]; cl_id = Some(id);
                                }
                            }
                        }
                    }
                ...
                match me.kind {
                    ...
                    MouseEventKind::Down(MouseButton::Right) => {
                        ...
                        if follow_body.is_some() { selected_body = follow_body; }
                    }
                    MouseEventKind::Down(MouseButton::Middle) => {
                        // LIMITADOR DE ASTEROIDES (Max 150 cuerpos para mantener el rendimiento O(N^2))
                        if bodies.len() < 150 {
                            let aim_dir = rotate_y(rotate_x(Vec3::new(0.0, 0.0, 1.0), cam_pitch), cam_yaw);
                            let target_offset = if let Some(id) = follow_body { bodies[id].pos } else { Vec3::new(0.0,0.0,0.0) };
                            
                            let spawn_pos = cam_pos.add(target_offset).add(aim_dir.mul(5.0));
                            let throw_vel = aim_dir.mul(25.0);
                            
                            let ast_name = format!("Asteroide-{}", asteroide_id_counter);
                            asteroide_id_counter += 1;
                            bodies.push(create_body(&ast_name, Color::BrightRed, 0.4, false, 50.0, spawn_pos, throw_vel, 2.0));
                        }
                    }
        ...
        if time_scale > 0.0 {
            ...
            for _ in 0..substeps {
                ...
                // 1. Primer medio-paso (Verlet): Actualizar Posiciones y pre-calcular Velocidad Media
                for b in bodies.iter_mut() {
                    ...
                    b.force = Vec3::new(0.0, 0.0, 0.0); // Reset de fuerzas para el nuevo cálculo
                }
                
                // 2. Calcular Fuerzas NUEVAS en las posiciones recién actualizadas
                for i in 0..n {
                    for j in i+1..n {
                ...
                // 3. Completar el paso de Velocidad con las nuevas fuerzas
                for b in bodies.iter_mut() {
                    if b.mass > 0.0 {
                        let acc = b.force.mul(1.0 / b.mass);
            ...
            for b in bodies.iter_mut() {
                if let Some(last) = b.trail.back() {
                    if b.pos.sub(*last).norm() > 0.8 { 
                        ...
                        if b.trail.len() > 100 { b.trail.pop_front(); } 
                    }
                } else {
                    b.trail.push_back(b.pos);
                }
                ...
                for p in body.trail.iter() {
                    if let Some(proj) = to_screen(*p) {
                        if let Some(prev) = prev_proj {
                            let line_color = if body.is_star { Color::Yellow } else { Color::BrightBlack };
        ...
        if show_help {
            ...
            chart_ref.text("[O]       Toggle Orbit Trails", 0.35, 0.60, Some(Color::White));
            ...
            chart_ref.text("[M-Click] FIRE ASTEROID (Max 150)", 0.35, 0.75, Some(Color::BrightRed));
            chart_ref.text("[X]       CLEAR ALL ASTEROIDS", 0.35, 0.80, Some(Color::BrightRed));
            chart_ref.text("[H]       Close this Help", 0.35, 0.85, Some(Color::BrightYellow));
        } else {
            let stress = format!("Monitor: {} FPS | Latencia: {}ms | Cuerpos: {}/150", current_fps, ms, bodies.len());
            ...
            if let Some(id) = selected_body {
                ...
                let sel_txt = format!("> SELECCIONADO: {} < | Masa: {:.0} | Distancia al Sol: {:.1} AU | {}", b.name, b.mass, dist, track_status);
                chart_ref.text(&sel_txt, 0.02, 0.14, Some(Color::BrightYellow));
            } else {
                let default_txt = format!("Clic Izq: Select | Clic Der: Seguir | Medio: DISPARAR | [X]: Borrar | {}", track_status);
                chart_ref.text(&default_txt, 0.02, 0.14, Some(Color::BrightBlack));
            }
        }

        execute!(stdout, cursor::MoveTo(0, 0))?;
        print!("{}", chart_ref.canvas.render_with_options(true, Some("Simulador FÍSICO Real (N-Cuerpos - Leapfrog)")).replace('\n', "\r\n"));
        stdout.flush()?;
