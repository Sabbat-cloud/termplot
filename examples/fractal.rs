...
// Definimos constantes para la resolución
...
fn main() {
    loop {
        ...
        println!("3. {}", "Atractor de Lorenz (Auto-scale)".yellow());
        println!("4. {}", "Helecho de Barnsley (Auto-scale)".yellow());
        println!("q. Salir");
        print!("\nOpción > ");
...
fn draw_julia() {
    ...
    // Parámetro C constante para Julia
    let c_re = -0.7;
    let c_im = 0.27015;
...
fn draw_lorenz() {
    ...
    for _ in 0..3000 {
        ...
        // Proyectamos en el plano X-Z (se ve mejor la forma de mariposa)
        points.push((x, z));
    }
